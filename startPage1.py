...
class StartPage1(tk.Frame):
    ''' This startPage 1 is use for other day store'''
    date = today
    time = str_now1
    ...
    def update_date(self):
        self.label1.configure(text="Available1 stores on " + self.date+ " ", font=self.controller.title_font1)
        self.after(10, self.update_date)

    def show_frame1(self, frame1, Date1, Time1, stall, name, bgimage, image, bg, menu):
        frame = frame1(parent=self.parent, controller=self.controller, date=Date1, time=Time1, stall = stall,
                 name = name, bgimage = bgimage, image = image, bg = bg, menu = menu)
